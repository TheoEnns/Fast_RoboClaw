...
{
	return ((uint16_t)((uint8_t)crc & 0x7F)<<8) + 0xFF;
	/*
	return crc;
	*/
}
	...
	do{
		...
		if(data!=-1){
			...
			if(data!=-1){
				...
				data = read(timeout);
				ccrc ^= 0x00FF;
				return crc_get()==ccrc;// changed to true; CRC is broken
			}
		}
	}while(trys--);
	...
	do{
		...
		if(data!=-1){
			...
			if(data!=-1){
				...
				data = read(timeout);
				ccrc ^= 0x00FF;
				if(crc_get()==ccrc){ // changed to true; CRC is broken
					*valid = true;
					return value;
				}
			}
			...
			if(data!=-1){
				...
				if(version[i]==0){
					...
					if(data!=-1){
						...
						data = read(timeout);
						ccrc ^= 0x00FF;
						/*Serial.print("crc_get(): ");
						Serial.println(crc_get(),DEC);
						Serial.print("ccrc: ");
						Serial.println(ccrc,DEC);*/
						return crc_get()==ccrc; // changed to true; CRC is broken
					}
					break;
				}
